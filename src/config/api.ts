...
export const callChangePassword = (currentPass: string, newPass: string) => {
  return axios.post<IBackendRes<string>>("/api/v1/auth/change-password", {
    currentPass,
    newPass,
  });
};
...
export const callUpdateUser = (user: IUserUpdate) => {
  return axios.put<IBackendRes<IUser>>(`/api/v1/users`, { ...user });
};

export const callUpdateUserByAdmin = (user: IUserUpdate) => {
  return axios.put<IBackendRes<IUser>>(`/api/v1/user-by-admin`, { ...user });
};
...
export const callSaveHr = (hr: IHrRegister) => {
...
export const callfetchHr = (query: string) => {
  return axios.get<IBackendRes<IModelPaginate<IHrRegister>>>(
    `/api/v1/hr-register?${query}`
  );
};
export const callGetHrById = (id: number) => {
  return axios.get<IBackendRes<IHrRegister>>(`/api/v1/hr-register/${id}`);
};
/**
 * 
Module Job
